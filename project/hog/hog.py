...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    ply, opp = player_score % 10, (opponent_score // 10) % 10
    res_0 = max(abs(opp - ply) * 3, 1)
    return res_0
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_avg_score = avg(1, dice)
    max_avg_rolls = 1
    for i in range(1, 10):
        avg_score = avg(i + 1, dice)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    # Not solved yet
    avg = make_averaged(roll_dice, samples_count=1000)
    zero_score = boar_brawl(score, opponent_score)
    if score > opponent_score:
        max_avg_rolls = 0
        if score + zero_score >= GOAL:
            return 0
        for i in [1, 2]:
            avg_score = avg(i, six_sided)
            if score + avg_score >= GOAL:
                return i
    avg_score = avg(6, six_sided)
